...
class HarmonicBundle:
	...
	def section_basis_V(self, p):
		...
		reduced = jnp.delete(linear_monomials, patch_idx, assume_unique_indices=True)
		blocks = [reduced] + [linear_monomials] * (self.rank_B - 1)
	...
	def loss_breakdown(self, data, params, bundle_metric_fn=None):
		...
		g_tr_F = hym.trace_F(data, params, self.curvature_form_fn, self._metric_fn)
		det_g_tr_F = jnp.linalg.det(g_tr_F)
		max_eig = vmap(jnp.linalg.norm)(g_tr_F)
		vol_Omega = jnp.mean(w)
		if self.rank_V > 1: g_tr_F = vmap(jnp.trace)(g_tr_F)
