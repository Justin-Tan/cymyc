...
def create_train_state(rng, g_model, f_model, optimizer, data_dim):
    rng, init_rng = random.split(rng)
    g_params = g_model.init(rng, jnp.ones([1, data_dim]))['params']
    f_params = f_model.init(rng, jnp.ones([1, data_dim]))['params']
    joint_params = {'g_model': g_params, 'f_model': f_params}
    opt_state = optimizer.init(joint_params)
    return joint_params, opt_state, init_rng

@partial(jit, static_argnums=(3,4,5))
def train_step(data, joint_params, opt_state, metric_fn, global_fn, pb_fn, optimizer):
    loss, dloss_dw = jax.value_and_grad(losses.perelman_objective_function, argnums=1)(data, joint_params, metric_fn, 
                                                                                       global_fn, pb_fn)
    param_updates, opt_state = optimizer.update(dloss_dw, opt_state, joint_params)
    joint_params = optax.apply_updates(joint_params, param_updates)
    return joint_params, opt_state, loss

def callback(epoch, t0, t, test_data, joint_params, metric_fn, global_fn, g_FS_fn, pb_fn,
             config, storage, logger, mode='TRAIN'):

    loss_breakdown_dict = losses.loss_breakdown_perelman(test_data, joint_params, metric_fn, global_fn, g_FS_fn, pb_fn, 
                                                         config.kappa, config.canonical_vol)
...
if __name__ == '__main__':
    ...
    *_, kmoduli, _ = config.poly_specification()
    ...
    joint_params, opt_state, init_rng = create_train_state(init_rng, model, optimizer, data_dim=config.n_ambient_coords * 2)
    # partial closure
    metric_fn = partial(models.ddbar_phi_model, g_ref_fn=g_FS_fn, g_correction_fn=g_correction_fn)
    global_fn = partial(models.phi_head, n_hyper=config.n_hyper, ambient=tuple(config.ambient))
    ...
    param_count = sum(x.size for x in jax.tree_util.tree_leaves(joint_params))
    logger.info(f'Params (Count: {param_count})=========>>>')
    logger.info(jax.tree_util.tree_map(lambda x: x.shape, joint_params))
    ...
    for epoch in range(int(config.n_epochs)):
        ...
        if (config.periodic_eval is False) and (epoch % config.eval_interval == 0):
            ...
            storage = callback(epoch, t0, 0, val_data, joint_params, metric_fn, global_fn, g_FS_fn, pb_fn, 
                               config, storage, logger, mode='VAL')
        ...
        for t, data in enumerate(wrapped_train_loader):
            joint_params, opt_state, loss = train_step(data, joint_params, opt_state, metric_fn, global_fn, pb_fn, optimizer)
        ...
        if epoch % config.save_interval == 0:
            utils.basic_ckpt(joint_params, opt_state, config.name, f'{epoch}')

    utils.basic_ckpt(joint_params, opt_state, config.name, 'FIN')
